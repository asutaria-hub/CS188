...
class LocalizationProblemTest(testClasses.TestCase):
    ...
    def solution(self, logicPlan):
        ...
        pacman.runGames(lay, pac, ghosts, disp, 1, False, catchExceptions=True, timeout=300)
...
class MappingProblemTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        if not self.check_len(grades, solution, gold_solution):
            return False

        for t in range(len(gold_solution)):
            if not self.check_len(grades, solution[t], gold_solution[t], "at time t = {}".format(t)):
                return False
...
class SLAMProblemTest(testClasses.TestCase):
    ...
    def solution(self, logicPlan):
        ...
        pacman.runGames(lay, pac, ghosts, disp, 1, False, catchExceptions=True, timeout=1800)
        return pac.planning_fn_output
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for soln_label, solution in zip(self.solution_labels, solutions):
            ...
            if not self.check_len(grades, solution, gold_solution, "for {}".format(soln_label)):
                return False

            for t in range(len(gold_solution)):
                if not self.check_len(grades, solution[t], gold_solution[t], "for {} at time t = {}".format(soln_label, t)):
                    return False
