...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        pool = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for elimination_var in eliminationOrder:
            pool, temp_factor = joinFactorsByVariable(pool, elimination_var)
            if len(temp_factor.unconditionedVariables()) == 1:
                continue
            pool.append(eliminate(temp_factor, elimination_var))
        return normalize(joinFactors(pool))
        "*** END YOUR CODE HERE ***"
